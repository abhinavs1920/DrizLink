...
func init() {
	...
	serverDir := filepath.Join(filepath.Dir(filepath.Dir(filepath.Dir(os.Args[0]))), "server")
	err := godotenv.Load(filepath.Join(serverDir, ".env"))
	if err != nil {
		// If not found in server dir, try current directory
		err = godotenv.Load()
		if err != nil {
